...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    nums.windows(25)
        .map(|window| window.iter().tuple_combinations())
        .zip(nums.iter().skip(25))
        .find_map(|(mut combines, &to_test)| {
            if combines.any(|(&v1, &v2)| to_test == v1 + v2) {
                None
            } else {
                Some(to_test)
            }
...
//         .find(|(idx, val)| {
//             sum += *val;
//             if sum > target && map.contains_key(&(sum - target)) {
//                 return true;
//             }
...
pub fn part_two(input: &str) -> Option<u64> {
    ...
    let nums = (2..)
        .flat_map(|ws| numbers.windows(ws))
        .find(|window| window.iter().fold(0, |sum, v| sum + *v) == target)
        .unwrap();
